...
export class DBService extends Observer {
    ...
    calcDataBasket() {
        ...
        let summa = 0, count = 0, skirt = 0, jacket = 0, trousers = 0;
    ...
    async addBasketInUserHistory(user: User | null): Promise<void> {
        if (!user || !this.dataUser) return;
        ...
        this.addBasketInAdminHistory(user);

        addDoc(collection(this.db, 'users', user.uid, 'history'), dataHistory)
            .then(async () => {
                await setDoc(doc(this.db, 'users', user.uid), newUser)
                    .then(() => {
    ...
    async getCounters() {
        const docRef = doc(this.db, "counters", "counters");
        return getDoc(docRef).then((doc): TCounter => {
            return doc.data() as TCounter;
        });
    }

    addBasketInAdminHistory(user: User | null) {
        if (!user || !this.dataUser) return;

        let dresses = this.dataUser.basket.filter(item => item.good.category === "платья").length;
        let jackets = this.dataUser.basket.filter(item => item.good.category === "куртки").length;
        let pants = this.dataUser.basket.filter(item => item.good.category === "брюки").length;

        // Получаем ссылку на документ счетчика покупок
        const docRef = doc(this.db, "counters", "counters");

        getDoc(docRef).then((doc) => {
            if (doc.id !== null) {

                const data = doc.data() as TCounter;

                let newDressesValue = data.dresses + dresses;
                let newJacketsValue = data.jackets + jackets;
                let newPantsValue = data.pants + pants;

                setDoc(docRef, {
                    dresses: newDressesValue,
                    jackets: newJacketsValue,
                    pants: newPantsValue
                }).then(() => {
                    console.error('Статистика успешно обновлена!');
                }).catch((error) => {
                    // Обработка ошибки
                })
            } else {
